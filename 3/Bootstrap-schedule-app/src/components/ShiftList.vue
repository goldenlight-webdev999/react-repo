<template>
  <b-row>
    <b-col cols="12">
      <h2>Shift List / This Weeks Schedule</h2>
      <b-btn href="#/addshift">Add New Shift</b-btn>
      <b-table striped hover :items="items" :fields="fields">
        <template slot="actions" scope="row">
          <b-btn size="sm" @click.stop="details(row.item)">Details</b-btn>
        </template>
      </b-table>
      <b-btn href="#/shiftlist2">Next Weeks Schedule</b-btn>
    </b-col>
  </b-row>
</template>
...
export default {
  ...
  methods: {
    details(schedule) {
      router.push({ name: "ShowShifts", params: { id: schedule.key } });
    }
  }
};
</script>
